...
# O(8^k) - Time Complexity
# O(8^k) - Space Complexity
def __recursive(n, k, cur_row, cur_column):
    ...
    for mov in knight_movements:
        total += __recursive(n, k - 1, cur_row + mov[0], cur_column + mov[1])

    return total


def knight_final_moves(n, k, row, column):
    return __prob(k, __recursive(n, k, row, column))
